...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'ExoGraphGUI.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'ExoGraphGUI.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'ExoGraphGUI.Client.ProtectedToken'
