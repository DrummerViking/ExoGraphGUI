...
$parentModule = 'ExoGraphGUI'
if (-not $ModuleName) { $ModuleName = 'ExoGraphGUI.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\ExoGraphGUI'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\ExoGraphGUI.psd1"
$prereqHash = @{
